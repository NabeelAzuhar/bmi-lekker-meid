...
function [x, y, modelParameters]= gloriaEstimator(testData, modelParameters)
    ...
    if timeTotal <= 560 % if total time is within the preset time bins
        ...
        % firingData = (2660x1) - test data

        % get classification weights from the model parameters for KNN
        weights = modelParameters.classify(binCount).wLDA_kNN;
        meanFiringRates = modelParameters.classify(binCount).mFire_kNN; % mean firing rate
        WTest = weights * (firingData - meanFiringRates); % test data projected onto LDA components
        WTrain = (modelParameters.classify(binCount).wOpt_kNN)'; % weights for LDA
        % WTest = (6x1)
        % WTrain = (6x800)
    ...
    if timeTotal <= 560
        ...
        xMean = modelParameters.averages(13).xMean(:, label);
        yMean = modelParameters.averages(13).yMean(:, label);
        xCoeff = modelParameters.pcr(label, 13).xM;

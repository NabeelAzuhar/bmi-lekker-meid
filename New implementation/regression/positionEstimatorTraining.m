...
% Initialisations
    ...
    PCA_threshold = 1; % The threshold for explaining the variance in PCA
    ...
    labels = labels(:); % 800 x 1 array with angle numbers
...
% 2. Find neurons with low firing rates for removal
    ...
    % Out: firingData (8*100, 98*28): rows = 8 angles * 100 trials, columns = 98 neurons * 28 bins

    % 2.2 Mark the neurons to be removed (with rates < 0.5)
    lowFiringNeurons = []; % list to store the indices of low-firing neurons
    for neuron = 1 : numNeurons
        avgRate = mean(mean(firingData(:, neuron:numNeurons:end)));
    ...
    removedCols = []; % columns of data to remove (for low firing neurons at all its time bins)
    for lowFirer = lowFiringNeurons
        removedCols = [removedCols, lowFirer:numNeurons:length(firingData)];
    end
    firingData(:, removedCols) = []; % remove the columns for the low firers
    ...
    %   firingData: filtered columns after neuron removal (removes column corresponding to low firing neurons)
...
% 3. Extract parameters for classification
    ...
    % 3.1 get firing rate data up to the certain time bin
        firingCurrent = firingData(:, 1:numNeuronsNew*interval);
        disp(size(firingCurrent))
    ...
    % 3.2 Principal Component Analysis for dimensionality reduction
        % Perform PCA on the neuron x time bins columns
        [currentComps, currentProjected, dimPCA1] = calcPCA(firingCurrent, PCA_threshold);
        disp(size(currentProjected));
        % currentComps is the principal components obtained after PCA
        % currentProjected is the firingCurrent data projected onto the principal components feature space

        dimPCA2 = size(currentProjected, 2);
...
%         optWeights = pcaProjection * eigenvectors(:, sortIdx(1:dimLDA)); % optimum parameters (95*timeBins x 6)
...
% Nested functions --------------------------------------------------------
    ...
    function [princeComps, projectedData, numComps] = calcPCA(data, explainedVariance)
        %----------------------------------------------------------------------
        % Manually computes the PCA. Reduced the dimension of the columns.

        % Arguments:
        %   data: firing data
        %   explainedVariance: A scalar value between 0 and 1 representing
        %       the desired percentage of variance to be explained by the 
        %       selected principal components.

        % Return Value:
        %   princeComps: A matrix containing the principal 
        %       components(eigenvectors) of the data. Each column 
        %       represents a principal component.
        %   projectedData: The original data projected onto the new 
        %       feature space defined by the selected principal components.
        %----------------------------------------------------------------------

        % Remove the mean from each feature (column)
        ...
        totalVariance = sum(diag(eigenvalues));
        explainedVarianceRatio = cumsum(diag(eigenvalues(indices))) / totalVariance;
    
        % Determine number of components to explain desired variance
        numComps = find(explainedVarianceRatio >= explainedVariance, 1);
    
        % Select top principal components
        princeComps = sortedEigenvectors(:, 1:numComps);
    
        % Project data onto new feature space
        projectedData = centeredData * princeComps;

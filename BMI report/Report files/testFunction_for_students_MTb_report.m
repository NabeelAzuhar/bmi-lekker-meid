...
function RMSE = testFunction_for_students_MTb_report(teamName)
...
tic;
% Train Model
modelParameters = positionEstimatorTrainingReport(trainingData);
...
for tr=1:size(testData,1)
    ...
    for direc=randperm(8) 
        ...
        for t=times
            ...
            if nargout('positionEstimatorReport') == 4
                [decodedPosX, decodedPosY, newParameters, label] = positionEstimatorReport(past_current_trial, modelParameters);
                modelParameters = newParameters;
            elseif nargout('positionEstimatorReport') == 3
                [decodedPosX, decodedPosY, label] = positionEstimatorReport(past_current_trial, modelParameters);
            end
